...
def main(script, direct):
    ...
    if platform.uname()[1].startswith('myers-mac-10'):
      print("Normal Sub.")
      job = r'python {} {}'.format(script, direct)
      ...
      job = "srun -J {2} -n 1 -p gpu --time=24:00:00 -e {1}/stderr -o {1}/stdout time python {0} {1} &".format(script, direct, os.path.basename(direct)[-8:])
      subprocess.call(job, shell=True)
      ...
      job = "bsub -J {2} -n 1 -q gpu -W 24:00 -e {1}/stderr -o {1}/stdout time python {0} {1} &".format(script, direct, os.path.basename(direct)[-8:])
      subprocess.call(job, shell=True)
    else:
